...
export class BaseView extends Backbone.View<Backbone.Model> {
  ...
  /**
   * Get Subview by tagName
   * @param  {String}   tagName (Tag of the component)
   * @return {BaseView}         (view stored in _views)
   */
  public getSubview(tagName: String): BaseView {
    if (!this._views) {
      this._views = [];
      console.warn('Uninitialized views array');
      return;
    ...
    if (!this._views.length) {
      this._views = [];
      console.warn('Views array is empty');
      return;
    }

    return _.find(this._views, (item) => {
      return item.el.tagName === tagName.toUpperCase();
    });
  }

  /**
   * Rendering subviews as component
   * @param {BaseView} view    (subview to render)
   * @param {String}   tagName (tag of the component)
   */
  public renderSubview(view: BaseView, tagName: String) {
    view.$el = this.$(tagName);
    view.render();
    view.delegateEvents();
  }
